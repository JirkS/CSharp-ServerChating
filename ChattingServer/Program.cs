    ...
    public class Program
    {
        ...
        private static void DoClient(object client)
        {
            ...
            do
            {
                ...
                if (text == "/quit")
                {
                    //State.LeaveTime = DateTime.Now;
                    Broadcast(string.Format("Connection from {0} closed.", name));
                    connections[Thread.CurrentThread.ManagedThreadId].LeaveTime = DateTime.Now;
                ...
                if (text == "/stats")
                {
                    SelfMessage(connections[Thread.CurrentThread.ManagedThreadId].IpAddress+ " \n"+ connections[Thread.CurrentThread.ManagedThreadId].JoinTime.ToString());
                }
        ...
        private static void SelfMessage(string text)
        {
            foreach (var oClient in connections)
            {
                if (oClient.Key == Thread.CurrentThread.ManagedThreadId)
